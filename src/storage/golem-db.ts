...
export class GolemDBStorage {
  private rpcUrl = process.env.GOLEM_DB_RPC_URL || 'https://kaolin.holesky.golemdb.io/rpc';
  private chainId = parseInt(process.env.GOLEM_DB_CHAIN_ID || '60138453025');

  async storeChunk(chunk: ChunkEntity): Promise<void> {
    const entity: GolemDBEntity = {
      id: chunk.id,
    ...
    try {
      ...
      console.error('Failed to store chunk to Golem DB:', error);
      throw error;
    }
  }
  ...
  async getChunk(media_id: string, chunk_index: number): Promise<ChunkEntity | null> {
    try {
      ...
      console.error('Failed to get chunk from Golem DB:', error);
      return null;
    }
  }
  ...
  async storeMetadata(metadata: MediaMetadata): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to store metadata to Golem DB:', error);
      throw error;
    }
  }
  ...
  async getMetadata(media_id: string): Promise<MediaMetadata | null> {
    try {
      ...
      console.error('Failed to get metadata from Golem DB:', error);
      return null;
    }
  }
  ...
  async getAllChunks(media_id: string): Promise<ChunkEntity[]> {
    ...
    try {
      ...
      console.error('Failed to get chunks from Golem DB:', error);
      return [];
    }
  }
  ...
  private async getCurrentBlock(): Promise<number> {
    try {
      ...
      console.error('Failed to get current block from Golem DB:', error);
