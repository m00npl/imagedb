...
export class GolemDBStorage {
  ...
  constructor() {
    this.chainId = parseInt(process.env.GOLEM_DB_CHAIN_ID || '60138453025');
    this.rpcUrl = process.env.GOLEM_DB_RPC_URL || 'https://kaolin.holesky.golem-base.io/rpc';
    this.wsUrl = process.env.GOLEM_DB_WS_URL || 'wss://kaolin.holesky.golem-base.io/ws';
    this.initialized = this.initializeClient();
  }

  private async initializeClient() {
    try {
      console.log('Creating Golem DB clients...');
      ...
      try {
        ...
        console.log('📊 Connected to Golem DB. Entity count:', entityCount);
      } catch (e) {
      ...
      console.log('Golem DB clients initialized successfully');
    } catch (error) {
      console.error('Failed to initialize Golem DB clients:', error);
      throw error;
    }
  }
  ...
  // Cleanup expired entries (optional - Golem DB should handle this automatically with BTL)
