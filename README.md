# ImagesDB - Image Chunking Middleware for Golem DB

> A middleware service that splits large images into safe-sized chunks, stores them on Golem DB, and serves them back as a single file from the user's point of view.

![ImagesDB Demo](https://img.shields.io/badge/demo-live-brightgreen) ![Docker](https://img.shields.io/badge/docker-ready-blue) ![Golem DB](https://img.shields.io/badge/Golem%20DB-integrated-purple)

## 🌟 Features

- **Large File Support**: Upload images up to 25MB
- **Automatic Chunking**: Files split into 64KB chunks that fit Golem DB limits
...
2. **Configure environment**
   ```bash
   cp .env.example .env
   # Edit .env with your Golem DB settings
   ```
...
| `GOLEM_DB_RPC_URL` | Golem DB RPC endpoint | `https://kaolin.holesky.golemdb.io/rpc` |
| `GOLEM_DB_CHAIN_ID` | Golem DB chain ID | `60138453025` |
...
services:
  imagedb:
    ...
    environment:
      - NODE_ENV=production
      - PORT=3000
      - GOLEM_DB_RPC_URL=https://kaolin.holesky.golemdb.io/rpc
...
- **Email**: [maciej.maciejowski@golem.network](mailto:maciej.maciejowski@golem.network)
